...
describe('initial render', () => {
  test('if document title renders', async () => {
    render(
      <Provider store={store}>
        <BrowserRouter>
          <UserRegisterPage />
...
describe('user attempts to register', () => {
  test('if user does not fill out form fields', () => {
  ...
  test('if user submits form with invalid email', async () => {
  ...
  test('if user submits a password less than 6 characters in length', async () => {
    const fakeUserLoginData = {
      firstName: 'Danny',
      lastName: 'Jacoshenk',
...
describe('golden path', () => {
  test('if user successfully submits form', async () => {
    ...
    expect(
      screen.queryByRole('alert', { name: /first name error/i })
    ).toBeNull();
    expect(
      screen.queryByRole('alert', { name: /last name error/i })
    ).toBeNull();
